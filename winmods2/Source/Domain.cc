...
const std::string& MaslGen::Domain::getName() const
{
  return getDomainName(analysis);
}
...
std::string MaslGen::Domain::getSignOff() const // Return domain Number, Name, KL, Version.
{
  return "end domain;\npragma number (" + boost::lexical_cast<std::string>(iUMLAPI::getDomainNumber(analysis)) + ");\n" +
                      "pragma name (\"" + getName() + "\");\n" +
                      "pragma kl (\"" + getKeyLetter() + "\");\n" +
	                  "pragma version (" + boost::lexical_cast<std::string>(iUMLAPI::getDomainVersionNumber(analysis)) + ");\n";
}

std::string MaslGen::Domain::getFileContents() const
{
  std::string modFile = getCopyrightNotice() + wrapDescription("",getDescription(),1000) + getDeclaration();
  ...
  {
    modFile += it->getModFileDeclaration("  ");
	modFile += "  pragma domain_operation_number (" + boost::lexical_cast<std::string>( it->getOperationNumber()) + ");" + "\n\n";
...
{
  std::string intFile = getCopyrightNotice() + wrapDescription("",domain.getDescription(),1000) + domain.getDeclaration();
