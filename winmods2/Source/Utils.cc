...
const std::string& iUMLAPI::getDomainName ( const iUMLAPI::XUML::AnalysisArea& analysisArea )
{
  return getDomainVersion(analysisArea).getIsAViewOfDomainR4().getName();
}
...
long iUMLAPI::getDomainVersionNumber ( const iUMLAPI::XUML::AnalysisArea& analysisArea )
{
  return getDomainVersion(analysisArea).getVersionNumber();
}
