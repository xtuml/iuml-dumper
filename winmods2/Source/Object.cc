...
{
  std::string result = wrapDescription(indent,clazz.getDescription(),1000) + 
        ...
        ++it )
  {
	...
	result += indent + "  " + "pragma operation_number (" + boost::lexical_cast<std::string>(it->getOperationNumber()) + ");\n\n";
  }
        ...
        ++it )
  {
	itCount++;
    result += it->getModFileDeclaration(indent + "  ") /* + "\n" */;
	result += indent + "  " + "pragma state_number (" + boost::lexical_cast<std::string>(it->getStateNumber()) + ");" + "\n\n";
        ...
        ++it )
  {
	...
	result += indent + "  " + "pragma event_number (" + boost::lexical_cast<std::string>(it->getSignalNumber()) + ");" + "\n\n" ;
  }
  ...
  for ( std::vector<std::pair<std::string,std::string> >::const_iterator it = tags.getTags().begin();
        ...
        ++it )
  {
	/* DNS - temporary change to eliminate clash with MASL keyword. Code in the 'else' was there before. */
	  if (makeIdentifier(it->first) == "array") 
	  {
          result+= indent + "pragma multiple (\"" + it->second + "\");\n";
	  }
	  else
	  {
